...
#include "stdio.h"
  ...
  bool calculateGradient3(const uint8_t *data, int row, int col, double *angles)
  {
    ...
    cv::Mat Angle = cv::Mat::zeros(row, col, CV_64FC1);
    cv::Mat Angle_masked = cv::Mat::zeros(row, col, CV_64FC1);
    cv::Mat Edge = cv::Mat::zeros(row, col, CV_8UC1);

    Canny(gray, Edge, 20, 60);

    Sobel(gray, DX, CV_32FC1, 1, 0, 3);
    Sobel(gray, DY, CV_32FC1, 0, 1, 3);

    // 计算梯度角度(角度制)
    phase(DY, DX, Angle, false);

    cv::Mat DX_flat = DX.reshape(1, DX.total() * DX.channels());
    std::vector<double> dx_array = DX_flat.isContinuous() ? DX_flat : DX_flat.clone();

    cv::Mat DY_flat = DY.reshape(1, DY.total() * DY.channels());
    std::vector<double> dy_array = DY_flat.isContinuous() ? DY_flat : DY_flat.clone();

    std::vector<double> angles_array;
    angles_array.resize(dy_array.size());
    for (size_t i = 0; i < dy_array.size(); i++)
    {
      angles_array[i] =
          atan2(dy_array[i], dx_array[i]); // calculate gradient
    }
    ...
    for (int idx = 0; idx < row; ++idx)
    {
      for (int idy = 0; idy < col; ++idy)
      {
        ...
        if (angles_array.at(idx * col + idy) > 0)
          angles[idx * col + idy] = angles_array.at(idx * col + idy);
        else
        {
    ...
    {
      cv::Mat Angle_out(row, col, CV_64FC1, angles);
      cv::imshow("angle out", Angle_out);
      imshow("gray", gray);
      imshow("angle", Angle);
      imshow("angle_masked", Angle_masked);
      imshow("canny", Edge);
    }
