...
namespace zgh
{
  ...
  bool calculateGradient3(const uint8_t *data, int row, int col, double *angles)
  {
    cv::Mat edge;
    cv::Mat DX, DY;
    cv::Mat gray = cv::Mat::zeros(row, col, CV_8UC1);
    cv::Mat Angle = cv::Mat::zeros(row, col, CV_64F);
    cv::Mat Edge = cv::Mat::zeros(row, col, CV_32F);
    ...
    for (int idx = 0; idx < row; ++idx)
    {
      ...
      {
        if (Edge.data[idx * col + idy] > 0)
          angles[idx * col + idy] = Angle.at<double>(idx, idy);
        else
        {
          angles[idx * col + idy] = ANGLE_NOT_DEF;
          Angle.at<double>(idx, idy) = ANGLE_NOT_DEF;
        }
      }
    }
    // imshow("dir", Angle);
